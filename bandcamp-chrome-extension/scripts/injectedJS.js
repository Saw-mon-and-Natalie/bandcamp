(function () {
  function init() {
    console.log(
      "%cbandcamp download%cSaw-mon & Natalie",
    ...
    const player = document.querySelector(
      "#trackInfoInner > div.inline_player > table > tbody"
    );
  ...
  function getDownloadLink(node) {
    const track_num =
      parseInt(node.getAttribute("rel").split("tracknum=")[1]) - 1;
    const link = window.TralbumData.trackinfo[track_num].file["mp3-128"];
    const title =
      window.TralbumData.artist +
      " - " +
      window.TralbumData.trackinfo[track_num].title;
    const track_id = window.TralbumData.trackinfo[track_num].track_id;
    return { href: link, title: title, track_id: track_id };
  }
  ...
  function addPlayerDownloadButton(node, childNode) {
    ...
    const chromeExtensionId = document
    ...
    container_a.addEventListener("click", function (e) {
      ...
      const title =
        window.TralbumData.artist +
        " - " +
        document.querySelector("span.title-section > a > span").innerText;

      chrome.runtime.sendMessage(chromeExtensionId, {
        message: "download_bandcamp",
        url: url,
        title: title,
      });
    });
  ...
  function addFullDivTracks(node, childNode) {
    ...
    if (childNode.querySelector(".play_status.disabled") == undefined) {
      ...
      const mp3TrackID = mp3Info.track_id;
      ...
      mp3Link.addEventListener("click", function (e) {
        ...
        var data = undefined;
        if (e.target.classList.contains("download_link_full")) {
          data = e.target.dataset;
        } else if (
          e.target.parentElement.classList.contains("download_link_full")
        ) {
